...
# Página 3 - Seleção da área e criação do campo
    ...
    with col2:
        ...
        if not st.session_state.field_created:
            create_button = st.button(t.get("createField", t["text_field5"]), disabled=not (sel and "selection" in sel and "box" in sel["selection"] and sel["selection"]["box"]))
            if create_button:
                ...
                y1_pdf = pdf_height - (y2_img * scale_y)
                ...
                can.acroForm.textfield(name="campo1", x=x1_pdf, y=y1_pdf, width=field_width, height=field_height, fillColor=transparent, borderStyle='inset', borderColor=transparent, borderWidth=0,forceBorder=True, textColor=HexColor(font_color), fontSize=font_size)
                can.save()
                packet.seek(0)
            ...
            with open(st.session_state.temp_pdf_result, "rb") as f:
                st.download_button(t.get("downloadPDF", t["text_field6"]), f, file_name="PDF_file.pdf", type="primary", icon=":material/download:")
